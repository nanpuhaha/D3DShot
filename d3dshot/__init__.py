...
def determine_available_capture_outputs():
    ...
    if numpy_is_available:
        available_capture_outputs.extend(
            (CaptureOutputs.NUMPY, CaptureOutputs.NUMPY_FLOAT)
        )

    if pytorch_is_available:
        available_capture_outputs.extend(
            (CaptureOutputs.PYTORCH, CaptureOutputs.PYTORCH_FLOAT)
        )

    if pytorch_gpu_is_available:
        available_capture_outputs.extend(
            (CaptureOutputs.PYTORCH_GPU, CaptureOutputs.PYTORCH_FLOAT_GPU)
        )
